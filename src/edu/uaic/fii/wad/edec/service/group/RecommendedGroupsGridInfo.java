...
public class RecommendedGroupsGridInfo extends AsyncTask<Void, Void, Void> {

    @Override
    protected Void doInBackground(Void... voids) {
        for (int i = 0; i < MainActivity.recommendationsGroups.size(); i++) {
            String id = MainActivity.recommendationsGroups.get(i).getId();
            ...
            if (jsonStr != null) {
                try {
                    ...
                    MainActivity.recommendationsGroups.get(i).setName(name);
                    MainActivity.recommendationsGroups.get(i).setLogo(logo);
